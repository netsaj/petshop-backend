...
func ContarServicios(c echo.Context) error {
	...
	if result := db.Model(models.HistoriaClinica{}).Where("terminado = false").Count(&historiaclinica); result.Error != nil {
		fmt.Print("error al contar HistoriaClinica")
		historiaclinica = 0
	}
